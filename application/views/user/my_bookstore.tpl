<div class="right_main">
	<dl class="mag_list clearfix">
		{if $is_me}
		<dt><a href="/user/me/bookstore" {if $type=='published'}class="sel"{/if}>已发布的杂志<span>({$bookstore[0].pub_total})</span></a>　|　<a href="/user/me/unpublished" {if $type=='unpublished'}class="sel"{/if}>未发布的杂志<span>({$bookstore[0].unpub_total})</span></a></dt>
		{/if}
